...
void JE_loadHelpText( void )
{
	...
	skip_strings_until_blank(f); // Surprisingly, we wind up using the other difficulty table instead

	// gameplay mode names
	//skip_pascal_string(f);
	//for (unsigned int i = 0; i < COUNTOF(gameplay_name); ++i)
	//	read_encrypted_pascal_string(gameplay_name[i], sizeof(gameplay_name[i]), f);
	skip_strings_until_blank(f); // Never used
...
/* No reason to dig any deeper into the file
	...
	skip_strings_until_blank(f); // Redundant
	...
	skip_strings_until_blank(f); // Unnecessary with ship select menu
	...
	skip_strings_until_blank(f); // Destruct was removed
	...
	//}
	skip_strings_until_blank(f); // Menus were removed
*/
