...
void JE_loadPals( void )
{
	...
	// Dynamically allocate palette memory to support T2000
	palettes = malloc(sizeof(Palette) * palette_count);
...
void fade_palette( Palette colors, int steps, unsigned int first_color, unsigned int last_color )
{
	...
	for (; steps > 0; steps--)
	{
		...
		// We ignore input, but hardcoded stuff like the service menu or coining up can still happen
		I_checkButtons();
	}
}
